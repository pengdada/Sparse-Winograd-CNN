...
def kernel_transform_num(kernel):
    ...
    GgGT = np.stack([g_[:, 0], g_[:, 0] / 2 + g_[:, 1] / 2 + g_[:, 2] / 2, g_[:, 0] / 2 - g_[:, 1] / 2 + g_[:, 2] / 2, g_[:, 2],
...
def image_transform(image, nl=tf.identity):
    ...
    CTdC = tf.concat([CTdC_0, CTdC_1, CTdC_2, CTdC_3, CTdC_4, CTdC_5, CTdC_6, CTdC_7, CTdC_8, CTdC_9, CTdC_10, CTdC_11, CTdC_12, CTdC_13, CTdC_14, CTdC_15], 0)
...
# image = np.reshape(np.linspace(0.0, float(batch_size * image_width *
# image_width * input_channel - 1) * 0.1, batch_size * image_width *
# image_width * input_channel), [batch_size, image_width, image_width,
# input_channel])
image_holder_1 = tf.placeholder(tf.float32,
                                shape=(batch_size, image_width, image_width, input_channel))

kernel = np.random.randn(3, 3, input_channel, output_channel)
kernel_holder = tf.placeholder(tf.float32,
...
transformed_kernel_holder = tf.placeholder(tf.float32,
...
transformed_image = tf.reshape(transformed_image, [16, batch_size, n_image_width, n_image_width, input_channel])

with tf.Session(config=tf.ConfigProto()) as sess_3:
    with tf.device('/gpu:0'):
        transformed_kernel_tf = tf.placeholder(dtype=tf.float32)
        my_winograd_conv_result = winograd2x2_conv.winograd2x2_conv(transformed_image, transformed_kernel_tf)

    sess_3.run(tf.initialize_all_variables())
    my_winograd_conv_result_num = sess_3.run(my_winograd_conv_result, {image_holder_1: image, transformed_kernel_tf: transformed_kernel_num})

print(np.sum(np.abs(conv_result_num - my_winograd_conv_result_num)) / (np.sum(np.abs(conv_result_num)) + (np.sum(np.abs(my_winograd_conv_result_num)))))
