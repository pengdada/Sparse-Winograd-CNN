...
output_grad = np.random.randn(16, batch_size, n_image_width, n_image_width, input_channel)

output_grad_holder = tf.placeholder(
	tf.float32,
	shape=(16, batch_size, n_image_width, n_image_width, input_channel))

transformed_image = tf.reshape(transformed_image, [16, batch_size, n_image_width, n_image_width, input_channel])

image_grad = tf.gradients(transformed_image, image_holder_1, grad_ys=output_grad_holder)

with tf.Session() as sess_2:
	image_grad_num = sess_2.run(image_grad, {output_grad_holder: output_grad})[0]
...
with tf.Session(config=tf.ConfigProto()) as sess_3:
	with tf.device('/gpu:0'):
		...
		my_image_grad = winograd2x2_imTrans.winograd2x2_imTrans_grad(output_grad_tf)

	sess_3.run(tf.initialize_all_variables())
	my_image_grad_num = sess_3.run(my_image_grad, {output_grad_tf: output_grad})

print np.sum(np.abs(image_grad_num - my_image_grad_num)) / (np.sum(np.abs(image_grad_num)) + (np.sum(np.abs(my_image_grad_num))))
